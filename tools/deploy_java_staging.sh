...
Deploy Apache Paimon Vector Index Java RC artifacts to Apache Nexus staging from a
...
Options:
  ...
  --repo REPO                GitHub repository. Defaults to apache/paimon-vector-index.
...
REPO=${REPO:-apache/paimon-vector-index}
...
if [[ -z "$STAGING_DESCRIPTION" ]]; then
  if [[ -n "$RC_NUMBER" ]]; then
    STAGING_DESCRIPTION="Apache Paimon Vector Index, version ${RELEASE_VERSION}, release candidate ${RC_NUMBER}"
  else
    STAGING_DESCRIPTION="Apache Paimon Vector Index, version ${RELEASE_VERSION}, release candidate ${TAG#*-rc}"
  fi
fi
...
copy_native \
  "$(find_native native-linux-x86_64/libpaimon_vindex_jni.so native/linux/x86_64/libpaimon_vindex_jni.so)" \
  native/linux/x86_64/libpaimon_vindex_jni.so \
  linux-x86_64
copy_native \
  "$(find_native native-linux-aarch64/libpaimon_vindex_jni.so native/linux/aarch64/libpaimon_vindex_jni.so)" \
  native/linux/aarch64/libpaimon_vindex_jni.so \
  linux-aarch64
copy_native \
  "$(find_native native-macos-aarch64/libpaimon_vindex_jni.dylib native/macos/aarch64/libpaimon_vindex_jni.dylib)" \
  native/macos/aarch64/libpaimon_vindex_jni.dylib \
  macos-aarch64
copy_native \
  "$(find_native native-windows-x86_64/paimon_vindex_jni.dll native/windows/x86_64/paimon_vindex_jni.dll)" \
